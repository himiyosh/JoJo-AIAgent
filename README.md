...
リポジトリ専用の **JoJoSlideAgent** を `.github/agents/JoJoSlideAgent.agent.md` に用意しています。Copilot CLI の `/agent` から選ぶか、`copilot --agent=JoJoSlideAgent --prompt "<依頼>"` で呼び出せます。Hallmark の anti-AI-slop 規律を、JoJo の `DESIGN.md` と固定 16:9／PDF／Reader の契約を優先しながらスライド向けに適用します。
