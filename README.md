...
- **スマホ Reader View（全33枚）**: <https://himiyosh.github.io/JoJo-AIAgent/reader/>
- **横型スマホ拡大pilot（Slide 06/16/28・比較用）**: <https://himiyosh.github.io/JoJo-AIAgent/mobile-pilot/>
...
- スマホReaderは、完成済みの16:9スライド全33枚を画像化・再編集せず、通常Slidevと同じDOMへReader操作を直接重ねて表示します。iframeや内側のカードは使わず、全体を確認してからピンチ／ダブルタップ／±で最大400%まで拡大し、拡大中はドラッグで移動できます。全画面操作は対応ブラウザでのみ表示します。
- **目次**では全33枚を「サムネイル＋章＋タイトル」の1列リストで確認して直接移動でき、見出し・本文・全タブ状態・発表者ノート・出典も検索できます。下部の**全体表示**は拡大を100%へ戻してスライド全体を画面内に収める操作で、下段に現在倍率を表示します。**補足**では現在のスライドの解説と出典を開けます。← → / PageUp / PageDown、`#slide-N`、ブラウザの戻る／進むにも対応します。
...
| **03** | いまの“動かし方”（ReAct → Harness → **Loop Engineering** → Graphの兆し） |
...
npm run qa:production -- --base /JoJo-AIAgent/       # 33枚・操作UI・Readerを検証
...
npm run qa:agent-context                              # 統括セッションの context circuit breaker を検証
...
| 旧縦向け Reader | canonical DOMから生成した33枚portrait版を`/reader-legacy/`へ比較用として維持 |
...
| 横型スマホ拡大pilot | Slide 06/16/28で旧iframe方式を比較できる検証用route。正式Readerは承認後にiframeを廃止し、通常Slidevへ直接統合済み |
...
JoJo-Git／JoJo-AIAgent 共通の **JoJoSlideAgent** を `.github/agents/JoJoSlideAgent.agent.md` に用意しています。Copilot CLI の `/agent` から選ぶか、`copilot --agent=JoJoSlideAgent --prompt "<依頼>"` で呼び出せます。現在のリポジトリの正本と QA を読み、Hallmark の anti-AI-slop 規律を固定キャンバス、PDF、Reader の契約へ合わせて適用します。
