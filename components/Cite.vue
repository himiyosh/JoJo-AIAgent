...
// Native details/summary keeps citations keyboard-operable without recreating
// disclosure semantics. Links stay in the DOM and expand for print/PDF export.
...
<template>
  <details class="cite" :class="[`cite--${place}`, `cite--pop-${popDir}`, { 'cite--now': now }]">
    <summary
      class="cite__summary"
      :aria-label="`出典 ${items.length}件を表示`"
      @click.stop
      @keydown.stop
    >
      ...
      <span class="cite__lab">出典 {{ items.length }}件</span>
      <span class="cite__toggle" aria-hidden="true">＋</span>
    </summary>
    <span class="cite__pop">
...
.cite__summary {
  ...
  min-height: 1.8rem; padding: .2em .55em; border-radius: 1em;
...
.cite__summary::-webkit-details-marker { display: none; }
.cite__i { opacity: .82; flex: none; }
.cite__toggle { margin-left: .12rem; color: var(--c); transition: transform .16s ease; }
.cite[open] .cite__toggle { transform: rotate(45deg); }
.cite__summary:hover, .cite__summary:focus-visible {
  color: var(--c); border-color: color-mix(in srgb, var(--c) 45%, var(--line));
  background: color-mix(in srgb, var(--c) 9%, transparent);
}
.cite__summary:focus-visible { outline: 3px solid var(--c); outline-offset: 3px; }
...
.cite--inline .cite__summary { min-height: 1.45rem; padding: .1em .45em; gap: .25em; }

/* ---- popover ---- */
.cite__pop {
  position: absolute; left: 50%; z-index: 40;
  width: max-content; max-width: min(34rem, 78vw);
  display: none; flex-direction: column; gap: .28rem;
...
.cite[open] .cite__pop {
  display: flex; opacity: 1; pointer-events: auto; transform: translate(-50%, 0);
}
.cite--tr[open] .cite__pop,
.cite--br[open] .cite__pop { transform: translate(0, 0); }
.cite--bar[open] .cite__pop { transform: translate(0, 0); }
...
@media (prefers-reduced-motion: reduce) {
  .cite__summary, .cite__toggle, .cite__pop { transition: opacity .1s ease; }
...
@media print {
  ...
  .cite__summary { display: none; }
  .cite__pop {
    display: flex !important; position: static; opacity: 1; transform: none !important;
